...
all:
	...
	rm -rf ${ENV_NAME} && virtualenv ${ENV_NAME} && . ${ENV_NAME}/bin/activate && pip install -r requirements.txt && pip install ${EXTRA_REQUIREMENTS} && cd test && echo `python --version` && export PYTHONPATH=../src && python -m unittest discover -s `pwd` -t `pwd` && cd .. && rm -r ${ENV_NAME}
