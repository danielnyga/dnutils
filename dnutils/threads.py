...
__all__ = ['active_count', 'Condition', 'current_thread', 'enumerate', 'Event',
           ...
           'Timer', 'ThreadError', 'ThreadInterrupt', 'setprofile', 'settrace', 'local']
...
_counter = _count()
next(_counter) # Consume 0 so first non-main thread has id 1.
        ...
        try:
            ...
            try:
                ...
                if _sys and _sys.stderr is not None:
                    ...
                    try:
                        _sys.stderr("Exception in thread %s (most likely raised during interpreter shutdown):" % self.name)
                        _sys.stderr("Traceback (most recent call last):")
                        while exc_tb:
                            _sys.stderr('  File "%s", line %s, in %s' % (exc_tb.tb_frame.f_code.co_filename,
                                                                         exc_tb.tb_lineno,
                                                                         exc_tb.tb_frame.f_code.co_name))
                            exc_tb = exc_tb.tb_next
                        _sys.stderr("%s: %s" % (exc_type, exc_value))
